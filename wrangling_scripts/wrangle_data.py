...
# fourth chart shows rural population vs arable land
    ...
    for country in countrylist:        
        graph_five.append(
            go.Bar(
                x = df[df['country'] == country].country.tolist(),
                y = df[df['country'] == country].totalruralpopulation.tolist()
            )
        )
    
    
    # TODO: fill a layout variable for the fifth visualization
    layout_five = dict(title = 'Total Rural Population by Country in 2015',
                      xaxis = dict(title = 'Country',),
